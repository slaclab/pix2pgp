...
-------------------------------------------------------------------------------
-- Description: SparkPix-Sv2 Wrapper
--
-- keeping it simple
...
entity Pix2PgpSparkPixSv2Top is
   generic(
      TPD_G                     : time      := 1 ns;
...
end entity Pix2PgpSparkPixSv2Top;

architecture rtl of Pix2PgpSparkPixSv2Top is
...
begin
   ...
   assert (NUM_OF_COL_MANAGERS_C = 24)
      report "[ERROR]: Pix2PgpSparkPixSv2Top; NUM_OF_COL_MANAGERS_C is *NOT* equal to 24! Please check that Pix2PgpPkg.vhd matches Pix2PgpSparkPixTPkg.vhd" severity failure;

   assert (ASIC_DATABUS_DWIDTH_C = 20)
      report "[ERROR]: Pix2PgpSparkPixSv2Top; ASIC_DATABUS_DWIDTH_C is *NOT* equal to 20! Please check that Pix2PgpPkg.vhd matches Pix2PgpSparkPixTPkg.vhd" severity failure;
   ...
   assert (cfgTimeoutLimit'length = TIMEOUT_LIMIT_WIDTH_C)
      report "[ERROR]: Pix2PgpSparkPixSv2Top; Please match cfgTimeoutLimit port width with TIMEOUT_LIMIT_WIDTH_C generic" severity failure;

   assert (cfgColumnEnable'length = NUM_OF_COL_MANAGERS_C)
      report "[ERROR]: Pix2PgpSparkPixSv2Top; Please match cfgColumnEnable port width with NUM_OF_COL_MANAGERS_C generic" severity failure;

   assert (sof'length = NUM_OF_COL_MANAGERS_C)
      report "[ERROR]: Pix2PgpSparkPixSv2Top; Please match sof port width with NUM_OF_COL_MANAGERS_C generic" severity failure;

   assert (eof'length = NUM_OF_COL_MANAGERS_C)
      report "[ERROR]: Pix2PgpSparkPixSv2Top; Please match eof port width with NUM_OF_COL_MANAGERS_C generic" severity failure;

   assert (overOcc'length = NUM_OF_COL_MANAGERS_C)
      report "[ERROR]: Pix2PgpSparkPixSv2Top; Please match overOcc port width with NUM_OF_COL_MANAGERS_C generic" severity failure;

   assert (pauseAck'length = NUM_OF_COL_MANAGERS_C)
      report "[ERROR]: Pix2PgpSparkPixSv2Top; Please match pauseAck port width with NUM_OF_COL_MANAGERS_C generic" severity failure;

   assert (wrEn'length = NUM_OF_COL_MANAGERS_C)
      report "[ERROR]: Pix2PgpSparkPixSv2Top; Please match wrEn port width with NUM_OF_COL_MANAGERS_C generic" severity failure;

   assert (busy'length = NUM_OF_COL_MANAGERS_C)
      report "[ERROR]: Pix2PgpSparkPixSv2Top; Please match busy port width with NUM_OF_COL_MANAGERS_C generic" severity failure;

   assert (pause'length = NUM_OF_COL_MANAGERS_C)
      report "[ERROR]: Pix2PgpSparkPixSv2Top; Please match pause port width with NUM_OF_COL_MANAGERS_C generic" severity failure;

   assert (FPGA_DATABUS_DWIDTH_C >= FPGA_PREAMBLE_LEN_C)
      report "[ERROR]: Pix2PgpSparkPixSv2Top; FPGA RX Preamble Length is larger than the width of the FPGA databus. Please check your Pkg file!" severity failure;

...
2. Run `make clean && make ASIC=<ASIC_NAME>` (run it while in the `firmware/` directory) at least *once*
...
In order to perform VHDL syntax-checking via GHDL, go to the `ghdl/` directory and run e.g. `$ bash ghdlRun.sh Pix2PgpSparkPixSTopTb` to perform a syntax check for the associated ASIC. Make sure you have done.

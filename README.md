...
2. While in the `firmware/` directory, run: `make clean && make ASIC=<ASIC_NAME>` at least *once*
    1. To import SparkPix-S, run `$ make clean && make ASIC=SparkPixS`
    2. To import SparkPix-T, run `$ make clean && make  ASIC=SparkPixT`
    3. To import Thriglav, run `$ make clean && make ASIC=Thriglav`
...
4. In your RTL analysis tool, parse everything inside the `firmware/asic/rtl` directory, including its subdirectories
    1. Do *not* link anything else (e.g. the contents of `firmware/vault`), as this will cause naming conflicts
5. If you need to import FPGA receiver design files, parse-in the contents of `firmware/fpga` accordingly
6. Note that the `make` commands above create symbolic links pointing to the surf submodule under the current repository (in 'firmware/submodules/surf'). If one wants to avoid nesting submodules they can invoke the `make` command with the `SURF_SOURCE_DIR=<surfDir>` flag added. This will link the surf libraries against an independent surf clone located under `<surfDir>` (e.g. `make ASIC=SparkPixS SURF_SOURCE_DIR=/sdf/home/u/user/project-dev/submodules/surf`)
...
In order to perform VHDL syntax-checking via GHDL, go to the `ghdl/` directory and run e.g. `$ bash ghdlRun.sh Pix2PgpSparkPixSTopTb` to perform a syntax check for the associated ASIC. Make sure you have done the `make ASIC=<ASIC_NAME>` in the `firmware/` dir first.

### How to simulate using GHDL
This is a defunct feature. If one wants to explore GHDL-based simulation anyway, the command that needs to be run in the `ghdl/` directory is: `$ bash ghdlRun.sh Pix2PgpSparkPixSTopTb 50`. Note the addition of the number at the end of the command (`50`), which will invoke the GHDL simulator for 50us of simulation time.
